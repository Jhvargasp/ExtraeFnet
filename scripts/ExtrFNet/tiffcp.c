...
tiffcp_interface(
		TIFF*	in,
		TIFF*	out,
		char*	cpCompressionOptions,
		uint16	cpConfig,
		uint16	cpFillorder,
		uint32	cpRowsperstrip,
		int		cpOutTiled,
		uint32	cpTilewidth,
		uint32	cpTilelength
	)
{
	config = cpConfig;
	fillorder = cpFillorder;
	rowsperstrip = cpRowsperstrip;
	outtiled = cpOutTiled;
	tilewidth = cpTilewidth;
	tilelength = cpTilelength;

	if (cpCompressionOptions != NULL && !processCompressOptions( cpCompressionOptions ))
		return 0;
...
/* SMinSampleValue & SMaxSampleValue */
	...
	case COMPRESSION_CCITTFAX4:
		...
		CopyField(TIFFTAG_XRESOLUTION, YResolution);
		if (XResolution>200 || YResolution>200) {
			XResolution=200.0;	
			TIFFSetField( in, TIFFTAG_XRESOLUTION, (float)XResolution );
			CopyTag(TIFFTAG_XRESOLUTION, 1, TIFF_RATIONAL);
			TIFFSetField( out, TIFFTAG_XRESOLUTION, (float)XResolution );
			YResolution=200.0;
			TIFFSetField( in, TIFFTAG_YRESOLUTION, (float)YResolution );
			CopyTag(TIFFTAG_YRESOLUTION, 1, TIFF_RATIONAL);
			TIFFSetField( out, TIFFTAG_YRESOLUTION, (float)YResolution );
		}
		break;
	}
	{ uint32 len32;
